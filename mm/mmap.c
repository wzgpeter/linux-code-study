	...
	struct vm_area_struct *expand)
{
	...
	if (next && !insert) {
		...
		if (end >= next->vm_end) {	//新区域的范围已经跨越整个后项vma，并且有可能超过后项vma的区域
			/*
			 ...
			 */
			if (next == expand) {	//next为后项vma，若相等表示：新区域要与后项vma进行合并 (vma_merge函数的后半部分会调用到这里)
				/*
				...
				swap(vma, next);
			} else {				//不相等表示：新区域要进行的是与前项vma进行合并的操作 (vma_merge函数的前半部分会调用到这里)
				VM_WARN_ON(expand != vma);
				/*
				 * case 1, 6, 7, remove_next == 2 is case 6,
				 * remove_next == 1 is case 1 or 7.
				 */
				remove_next = 1 + (end > next->vm_end);	//新区域确实超过了后项vma的区域
...
struct vm_area_struct *vma_merge(struct mm_struct *mm,
			...
			struct vm_userfaultfd_ctx vm_userfaultfd_ctx)
{
	pgoff_t pglen = (end - addr) >> PAGE_SHIFT;		//计算出新的vma区域的大小，单位是页(page)
	...
	if (prev)
		next = prev->vm_next;		//指定了前项vma，则获取前项vma的后项vma
	else
		next = mm->mmap;			//没有指定前项vma，则mm->mmap指向第一个元素就是后项vma
	area = next;
	 ...
	 */
	if (prev && prev->vm_end == addr &&					//插入的vma区域的起始地址与前一区域的结束地址相等
			mpol_equal(vma_policy(prev), policy) &&		//在NUMA的CPU框架下，两个区域的vma_policy属性相同
			can_vma_merge_after(prev, vm_flags,			//两者的vm_flags相同以及是否映射同一个文件
					    anon_vma, file, pgoff,
					    vm_userfaultfd_ctx)) {

		//此处表明当前区域可以和前项vma合并了
		//后面再判断当前区域是否可以和后项vma合并
		/*
		 * OK, it can.  Can we now merge in the successor as well?
		 */
		if (next && end == next->vm_start &&			//插入的vma区域的截止地址与后项vma的起始地址是否相等
				mpol_equal(policy, vma_policy(next)) &&	//在NUMA的CPU框架下，两个区域的vma_policy属性相同与否
				can_vma_merge_before(next, vm_flags,	//两者的vm_flags相同以及是否映射同一个文件
						     anon_vma, file,
						     pgoff+pglen,
						     vm_userfaultfd_ctx) &&
	...
	//运行到此处表明：
	//当前区域不能和前项vma进行合并，则按照与前项区域进行合并的方法类似，
	//判断当前区域是否可以和后项vma区域进行合并
		...
		struct list_head *uf)
{
	...
	} else if (vm_flags & VM_SHARED) { /* 为非文件映射，即匿名映射 */
